...
export const listPostLikes = new Elysia().use(middlewares).get(
  '/posts/:postId/likes',
  async ({ params, query, status }) => {
    const { postId } = params
    const { cursor, limit } = query

    const key = `posts:${postId}:likes`
    ...
    const likes = await db
      ...
      .where(
        and(
          eq(schemas.likes.postId, postId),
  ...
  },
  {
    ...
    detail: {
      ...
      description: 'List likes for the specified post',
