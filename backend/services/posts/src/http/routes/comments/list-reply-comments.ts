...
export const listReplyComments = new Elysia().use(middlewares).get(
  '/comments/:commentId/replies',
  async ({ params, query, status }) => {
    const { commentId } = params
    const { cursor, limit } = query

    const key = `comments:${commentId}:replies`
    ...
    const comments = await db
      ...
      .where(
        and(
          eq(schemas.comments.replyId, commentId),
  ...
  },
  {
    ...
    detail: {
      ...
      description: 'List comments by reply Id',
      operationId: 'listReplyComments',
    },
    params: z.object({
      commentId: z.string(),
    }),
