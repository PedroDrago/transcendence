...
export const countStoryComments = new Elysia().use(middlewares).get(
  '/stories/:storyId/comments/count',
  async ({ params, status }) => {
    const { storyId } = params

    const key = `stories:${storyId}:comments:count`
  ...
  },
  {
    ...
    detail: {
      tags: ['Comments'],
      summary: 'Count story comments',
      description:
        'Get the total number of top-level comments for the specified story',
      operationId: 'countStoryComments',
    },
    params: z.object({
      storyId: z.uuidv7(),
    }),
